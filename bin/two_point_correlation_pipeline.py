...
FILE_TEMPLATE = Template("two_point_corr/raw/T=${timestep}_LINE${number}.csv")
VARIABLES = ("U", "UMean")
PI = np.pi
...
def run(point1, point2, lines_per_timestep,
        line_resolution, timesteps, output_dir):
    ...
    path = os.path.join(output_dir, "two-point-corr-extraction.json")
    dump_to_json({"lines": lines}, path, indent=4)


def main():
    args = parse_args()
    run(args.point1, args.point2, args.num_lines_per_sample,
        args.line_resolution, args.timesteps_to_sample, args.output_dir)
